...
export const generateBrianResponse = async (
  ...
  // FIX: Per guidelines, API key must be retrieved from process.env.API_KEY.
  const apiKey = process.env.API_KEY;

  if (!apiKey) {
    // FIX: Updated warning message to reference API_KEY.
    console.warn("API_KEY not found. Please check your .env file or Netlify environment variables.");
