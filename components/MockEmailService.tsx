...
const MockEmailService: React.FC = () => {
  ...
  useEffect(() => {
    // Strictly ONLY show if NOT in production
    if (!SYSTEM_IDENTITY.IS_PRODUCTION && lastEmail) {
      setVisible(true);
    } else {
      setVisible(false);
    }
  ...
  if (SYSTEM_IDENTITY.IS_PRODUCTION || !lastEmail || !visible) return null;
    ...
    <div className="fixed top-20 right-4 z-50 max-w-sm w-full bg-white rounded-xl shadow-2xl border border-gray-200 overflow-hidden animate-slide-in-right ring-1 ring-black/5">
      <div className="bg-blue-600 px-4 py-3 flex justify-between items-center text-white">
        <div className="flex items-center gap-2">
           ...
           <span className="font-bold text-sm">Novo Email (Debug)</span>
        </div>
