      ...
      <div className="bg-white shadow-sm border-b border-gray-200 sticky top-16 z-30">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
            ...
            <div className="mt-4 pt-4 border-t border-gray-100 flex flex-wrap gap-3">
                <select value={filterService} onChange={e => setFilterService(e.target.value)} className="bg-gray-50 border border-gray-200 text-gray-700 py-2 px-4 rounded-xl text-sm font-bold focus:ring-2 focus:ring-blue-500 outline-none">
                    <option value="All">All Service Types</option>
                    {Object.entries(SERVICE_UI_MAP_EN).map(([key, label]) => (
                        <option key={key} value={key}>{label}</option>
                    ))}
                </select>
                <select value={filterExperience} onChange={e => setFilterExperience(e.target.value)} className="bg-gray-50 border border-gray-200 text-gray-700 py-2 px-4 rounded-xl text-sm font-bold focus:ring-2 focus:ring-blue-500 outline-none">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {isLoading ? (
           <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {[...Array(6)].map((_, i) => (<div key={i} className="h-[450px] bg-white rounded-3xl animate-pulse border border-slate-100"></div>))}
           </div>
        ) : results.length === 0 ? (
            ...
            {results.map((cleaner) => (
              <div key={cleaner.id} className="bg-white rounded-[32px] shadow-sm hover:shadow-2xl transition-all duration-500 border border-gray-100 flex flex-col overflow-hidden group transform hover:-translate-y-1">
                <div className="h-64 bg-gray-200 relative overflow-hidden">
                  ...
                  {cleaner.level !== CleanerLevel.BRONZE && (<div className="absolute top-4 right-4"><LevelBadge level={cleaner.level} /></div>)}
                </div>
                <div className="p-8 flex-1 flex flex-col">
                    ...
                    <div className="flex flex-wrap gap-1.5 mb-8">
                        {cleaner.services.slice(0, 4).map(s => (<span key={s} className="bg-slate-50 text-slate-500 text-[9px] font-black uppercase px-2 py-1 rounded-lg border border-slate-100 tracking-tighter">{SERVICE_UI_MAP_EN[s] || 'Standard Clean'}</span>))}
