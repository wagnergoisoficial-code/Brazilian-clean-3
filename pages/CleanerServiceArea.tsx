...
const CleanerServiceArea: React.FC = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!cleanerId || zips.length === 0) return alert("Selecione ao menos um CEP.");
    updateCleanerProfile(cleanerId, { zipCodes: zips });
    navigate('/dashboard');
  };
    ...
    <div className="min-h-screen bg-teal-50 py-12 px-4 flex items-center justify-center font-sans">
      <div className="max-w-lg w-full bg-white rounded-3xl shadow-2xl overflow-hidden animate-scale-in">
        <div className="bg-slate-900 py-12 px-8 text-center text-white">
           ...
           <p className="text-slate-400 mt-2">Onde você realiza seus serviços?</p>
        </div>
        <div className="p-8 space-y-6">
          <div className="space-y-4">
            <label className="block text-xs font-black uppercase text-slate-400">Adicionar ZIP Code</label>
            <div className="flex gap-2">
                <input type="text" maxLength={5} className="flex-1 bg-slate-50 border-2 border-slate-50 rounded-2xl p-4 outline-none focus:border-blue-500 transition" value={zipInput} onChange={e => setZipInput(e.target.value.replace(/\D/g,''))} placeholder="Ex: 32801" />
                <button onClick={addZip} className="bg-slate-900 text-white px-6 rounded-2xl font-bold">Add</button>
            </div>
          </div>
          <div className="flex flex-wrap gap-2">
              {zips.map(z => (
                  <span key={z} className="bg-blue-50 text-blue-600 px-4 py-2 rounded-xl font-bold flex items-center gap-2 border border-blue-100">
                      {z}
                      <button onClick={() => removeZip(z)} className="text-blue-300 hover:text-red-500">✕</button>
                  </span>
              ))}
          </div>
          <button onClick={handleSubmit} className="w-full bg-blue-600 text-white py-5 rounded-2xl font-black uppercase tracking-widest text-sm shadow-xl">Salvar e Continuar</button>
        </div>
      </div>
    </div>
  );
